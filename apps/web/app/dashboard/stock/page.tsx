...
type ItemStock = {
  ...
  ultimo_movimento_tipo: string | null
}
...
export default function PaginaStock() {
  ...
  async function carregarStock() {
    ...
    setErro('')
    try {
      const dados = await api.stock.doFornecedor()
      setItens(dados as ItemStock[])
    } catch (err) {
  ...
  function abrirPainel(peca: ItemStock) {
    // Fechar painel anterior ou abrir novo
  ...
  async function confirmarAjuste(peca: ItemStock) {
    if (!painel) return
    setPainel(prev => prev ? { ...prev, carregando: true, erro: '' } : null)
    try {
      await api.stock.ajustar(peca.id, {
        estoque: parseInt(painel.estoqueNovo),
        tipo: painel.tipo,
        observacao: painel.observacao || undefined,
      })
      // Recarregar lista e movimentos
      const [novosItens, movimentos] = await Promise.all([
        api.stock.doFornecedor(),
        api.stock.movimentos(peca.id),
      ])
      setItens(novosItens as ItemStock[])
      setPainel(prev => prev ? {
        ...prev,
        carregando: false,
        movimentos: (movimentos as Movimento[]).slice(0, 5),
        estoqueNovo: String((novosItens as ItemStock[]).find(i => i.id === peca.id)?.estoque ?? painel.estoqueNovo),
      } : null)
    } catch (err) {
  ...
  if (carregando) {
    return (
      <div className="p-4 sm:p-6">
        <h1 className="text-xl font-bold text-[#111111] mb-6">Gestão de Stock</h1>
        <div className="space-y-3">
          {[1, 2, 3].map(n => (
            <div key={n} className="h-20 bg-gray-100 animate-pulse rounded-xl" />
          ))}
        </div>
      </div>
    )
  ...
  return (
    <div className="p-4 sm:p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-bold text-[#111111]">Gestão de Stock</h1>
        <button
          onClick={carregarStock}
      ...
      {itens.length === 0 && !erro && (
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="font-medium">Sem peças em stock</p>
          <p className="text-sm mt-1">Crie a sua primeira peça no separador Peças.</p>
        </div>
      )}
          ...
          <div key={peca.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-1 flex-wrap">
                  <span className={`text-sm ${corEstoque(peca.estoque)}`}>
                    {labelEstoque(peca.estoque)} {peca.estoque > 0 && peca.estoque > 2 ? 'unid.' : ''}
                  </span>
            ...
            {painel?.pecaId === peca.id && (
              <div className="border-t border-gray-100 bg-gray-50 p-4 space-y-3">
                <p className="text-sm font-medium text-gray-700">Ajuste de stock</p>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ex: Vendido por WhatsApp"
                    maxLength={500}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-[#dc2626]"
                  />
                </div>
                ...
                {painel.erro && (
                  <p className="text-sm text-red-600">{painel.erro}</p>
                )}
